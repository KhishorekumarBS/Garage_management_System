	...
	public function saveUpdateRepairCarInformation($con, $data, $image_url) {
		if(!empty($data)) {
			if($data['repair_car'] == '0') {
				mysqli_query($con,"INSERT INTO tbl_add_car(repair_car_id,car_name,customer_id,car_make,car_model,year,chasis_no,car_reg_no,VIN,note,added_date,image) values('$data[hfInvoiceId]','$data[car_names]','$data[ddlCustomerList]','$data[ddlMake]','$data[ddlModel]','$data[ddlYear]','$data[car_chasis_no]','$data[registration]','$data[vin]','$data[car_note]','$data[add_date]','$image_url')");
			} else {
